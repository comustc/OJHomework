...
<body>
	...
	<div class="main">
		...
		<div class="list" id="clazz_list">
			...
			<div class="search form-inline">
				<form action="admin/clazz/list" method="post"
					...
					</select> <input type="submit" class="btn btn-default btn-xs" value="搜索">
				</form>
			</div>
			<!--操作按钮-->
			...
			<input type="button" onclick="delAllclass()" value="批量删除" />
			<table class="table table-hover">
				<thead>
					<tr id="lsit">
						<td><input type="checkbox" onclick="funcCheckAll()" id="checkboxall"></td>
						<th width="15%">id</th>
						<th width="20%">年级</th>
						<th width="30%">专业</th>
						<th width="20%">班级</th>
				...
				<tbody>
					...
					<c:forEach items="${pageBean.records}" var="clazz">
						<tr>
							<td>
							<input type="hidden" id="subcheck" name="ids" value="0" /> 
							 <input
								id="subcheck" type="checkbox" name="ids"
								onclick="checkReturn(this)" value="${clazz.id}"></input></td>
							<td>${clazz.id}</td>
							...
							<td>
								<button class="btn btn-danger btn-xs"
									onclick="deleteClazz(this);">删除</button>
							</td>
						</tr>
					</c:forEach>
				</tbody>
	...
	<div class="modal_window clazz_window form-control" id="clazz_add">
		...
		<form action="admin/clazz/add" method="post"
			onsubmit="return addClazz(this);">
			<table>
				<tr>
					<td>所属年级:</td>
					<td><select name="grade" onchange="changeMajor(this);">
							<option value="0">年级...</option>
							<c:forEach items="${grades}" var="grade">
								<option value="${grade.id}">${grade.grade}</option>
							</c:forEach>
				...
				<tr>
					<td>所属专业:</td>
					<td>
						<!-- 此处由ajax动态加载 --> <select id="major_select" name="major">
							<option value="0">专业...</option>
					</select>
					</td>
				</tr>
